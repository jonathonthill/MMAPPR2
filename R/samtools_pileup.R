.samtoolsPileup = function(files, param, chrRange) {
    ...
    for (file in BiocGenerics::path(files)) {
        ...
        pos = pile_dat$V2
        counts = rbind(str_count(pile_dat$V5, "A"),
                       str_count(pile_dat$V5, "C"),
                       str_count(pile_dat$V5, "G"),
                       str_count(pile_dat$V5, "T"),
                       cvg = pile_dat$V4)
        counts = as.vector(counts)
        countsdf = data.frame(call = c("A", "C", "G", "T", "cvg"), pos = rep(pos, each = 5), counts = counts)
        
        # Assemble
        if (exists("outdat")) {
            outdat = merge(outdat, countsdf, by = c("pos", "call"), all = TRUE, sort = FALSE)
        } else {
            outdat = countsdf
        }
    }
